...
class RegistrationPhoneNumberScreen extends StatefulWidget {
  ...
  @override
  State<RegistrationPhoneNumberScreen> createState() =>
      _RegistrationPhoneNumberScreenState();
}

class _RegistrationPhoneNumberScreenState
    extends State<RegistrationPhoneNumberScreen> {
              ...
              child: Column(
                ...
                TextField(
                  ...
                  decoration: customInputDecoration(
                      true, LocaleKeys.registration_type_phone_hintPass.tr()),
                  onChanged: (value) {
                    setState(() {
                      passwordError = false;
                    });
                ...
                TextField(
                  ...
                  enabled: _isLoadingComplete,
                  obscureText: !_passwordVisible,
                  decoration: customInputDecoration(true,
                      LocaleKeys.registration_type_phone_hintSecPass.tr()),
                  onChanged: (value) {
                    setState(() {
                      passwordError = false;
                    });
    ...
    return SizedBox(
        ...
        child: MaterialButton(
          ...
          onPressed: _isLoadingComplete
              ? () {
                  if (_secondPasswordTextController.text !=
                          _firstPasswordTextController.text ||
                      _secondPasswordTextController.text == "") {
                    setState(() {
                      passwordError = true;
                      errorMessage =
                          LocaleKeys.registration_error_diffPass.tr();
                    });
                    return;
                  }
  ...
  _sendRequestPostBodyHeaders() async {
    String phoneNumber = _numberTextController.text
        .replaceAll("+", "")
        .replaceAll(" ", "")
        .replaceAll("-", "");
    var item =
        await PhoneNumber.getRegionInfoFromPhoneNumber(phoneNumber, "RU");
    phoneNumber = item.phoneNumber.toString().replaceAll("+", "");
    log("phone:${phoneNumber}");
    var response = await http.post(
      Uri.parse(NetworkService().baseUrl + NetworkService().registration),
      body: jsonEncode(
        {
      ...
      headers: {
        ...
        'X-CSRF-TOKEN': '77Ln6M0rcMQadYCDHOMF9ooKRG8Kg0GFwWB1PvT6'
      },
    );
  ...
  InputDecoration customInputDecoration(bool needSuffixIcon, String hintText) {
    return InputDecoration(
      suffixIcon: (needSuffixIcon == true)
          ? IconButton(
              onPressed: () {
                setState(() {
              ...
              icon: Icon(
                _passwordVisible ? Icons.visibility_off : Icons.remove_red_eye,
                color: Colors.grey,
              ))
          : null,
